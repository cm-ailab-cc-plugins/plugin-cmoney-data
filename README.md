...
Claude Code plugin — 整合 Anya 使用者行為資料 + Readmo GA4 查詢。

OAuth 授權後讓 Claude Code 直接：
- 生 Spark SQL 查 Anya（SDK 埋點事件，例如 launch/click）
- 打 Readmo GA4 report API（page_view、CTR、AB 測試比較）

每次查詢都寫 audit log，Anya / GA4 共用 60 req/min rate limit。

## 安裝

```
/plugin install cmoney-data@cm-ailab-cc-plugins
```
...
skill (`skills/cmoney-data/SKILL.md`) 會引導 Claude Code 自動判斷該走 Anya 還是 GA4 路徑。
